...
function void_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'voidgrid' ),
	...
	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'voidgrid' ),
...
function void_entry_header() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'voidgrid' ) );
		if ( $categories_list && void_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( ' %1$s ', 'voidgrid' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'voidgrid' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( ' %1$s', 'voidgrid' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'voidgrid' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
